...
setInterval(() => {
  ...
  for (const [roomId, room] of gameRooms.entries()) {
    ...
    if (
      (room.createdAt && now - room.createdAt > 2 * 60 * 60 * 1000) ||
...
io.on("connection", (socket) => {
  ...
  socket.on("createRoom", ({ playerName, difficulty, gameTheme }) => {
    console.log(
      `Creating room for ${playerName} with difficulty ${difficulty} and theme ${gameTheme}`
    );
  ...
  socket.on("joinRoom", ({ roomId, playerName, gameTheme }) => {
    ...
    if (!room) {
      ...
      console.log(
        `Available rooms: ${Array.from(gameRooms.keys()).join(", ")}`
      );
    ...
    if (
      room.gameState.gameTheme &&
      gameTheme &&
      room.gameState.gameTheme !== gameTheme
    ) {
      console.log(
        `Theme mismatch: room=${room.gameState.gameTheme}, client=${gameTheme}`
      );
    ...
    // Enhance this section to explicitly include difficulty in the response:
    console.log(
      `Player joined room ${formattedRoomId} with difficulty ${room.gameState.difficulty}`
    );
    ...
    socket.emit("roomJoined", {
      ...
      gameState: room.gameState, // This includes difficulty, but let's be explicit too
      difficulty: room.gameState.difficulty, // Explicitly include difficulty
      players: room.players,
      isHost: false,
    });
  ...
  socket.on(
    "startGame",
    ({ roomId, cards, layoutConfig, gameTheme, difficulty }) => {
      ...
      if (layoutConfig) {
        ...
        console.log(
          `Storing layout config: ${layoutConfig.cols}x${layoutConfig.rows}`
        );
      }
  ...
  // Handle card click
  // In the part where you handle the cardClick event
  socket.on("cardClick", ({ roomId, cardIndex, gameTime }) => {
    const room = gameRooms.get(roomId);
    ...
    // Update room's game time if provided
    if (gameTime !== undefined && typeof gameTime === "number") {
      room.gameState.gameTime = gameTime;
    }
    ...
    io.to(roomId).emit("cardFlipped", {
      ...
      gameTime: room.gameState.gameTime, // Include the current game time
    });
    ...
    if (gameState.flippedIndices.length === 2) {
      ...
      if (gameState.cards[firstIndex].id === gameState.cards[secondIndex].id) {
        ...
        console.log(
          `Match found! Player ${playerIndex} found a pair. Score: ${room.players[playerIndex].score}`
        );
        ...
        setTimeout(() => {
          ...
          if (gameState.matchedPairs.length === gameState.cards.length / 2) {
            console.log(
              `Game over in room ${roomId}, final time: ${room.gameState.gameTime}`
            );
            ...
            io.to(roomId).emit("gameOver", {
              ...
              players: room.players,
              message: "Game Over!",
              gameTime: room.gameState.gameTime, // Send the final time to all players
            });
          } else {
  ...
  // Update the startGame handler to ensure everyone gets the same card deck
  socket.on(
    "startGame",
    ({ roomId, cards, layoutConfig, gameTheme, difficulty }) => {
      console.log(`Starting game in room: ${roomId}`);
      ...
      room.gameState.status = "playing";
      room.gameState.currentPlayerIndex = 0;
      room.gameState.gameTime = 0; // Reset game time
      ...
      io.to(roomId).emit("gameStarted", {
        ...
        gameTheme: room.gameState.gameTheme,
        difficulty: room.gameState.difficulty,
        layoutConfig: room.gameState.layoutConfig,
        message: "Game has started!",
      });
    }
  );

  socket.on('syncGameTime', ({ roomId, gameTime }) => {
    const room = gameRooms.get(roomId);
    ...
    // Only allow the host to update the game time
    if (socket.id === room.host) {
      // Update the room's game time
      room.gameState.gameTime = gameTime;
      
      // Broadcast the updated time to all players
      io.to(roomId).emit('gameTimeSync', {
        roomId,
        gameTime: room.gameState.gameTime
      });
    }
  ...
  socket.on('playAgain', ({ roomId }) => {
    ...
    // Store the current player state to preserve player information
    const preservedPlayers = JSON.parse(JSON.stringify(room.players));
    ...
    // Reset player scores but preserve other player information
    preservedPlayers.forEach(player => {
      player.score = 0;
      player.moves = 0;
    });
    
    // Update the players array with the preserved data
    room.players = preservedPlayers;
    
    console.log(`Game reset in room ${roomId}`);
    console.log(`Players after reset:`, room.players);
    
    // Make sure all players are properly in the room
    room.players.forEach(player => {
      const playerSocket = io.sockets.sockets.get(player.id);
      if (playerSocket) {
        playerSocket.join(roomId);
        console.log(`Ensuring player ${player.id} is in room ${roomId}`);
      } else {
        console.log(`Warning: Could not find socket for player ${player.id}`);
      }
    });
    
    // Notify all players about restart with all necessary data
    ...
    // Send an updated player list event separately to ensure clients update their player state
  ...
  socket.on("requestPlayerList", ({ roomId }) => {
    ...
    // Send current player list to the requesting client
  ...
  socket.on("updateDifficulty", ({ roomId, difficulty }) => {
    console.log(`Difficulty update request for room ${roomId}: ${difficulty}`);
    ...
    if (socket.id !== room.host) {
      console.log(`Non-host tried to update difficulty: ${socket.id}`);
      socket.emit("roomError", {
        message: "Only the host can update difficulty",
      });
      return;
    }

    // Update room difficulty
    room.gameState.difficulty = difficulty;
    console.log(`Room ${roomId} difficulty updated to ${difficulty}`);

    // Notify all players in the room about the difficulty change
    io.to(roomId).emit("difficultyUpdated", {
      roomId,
      difficulty,
      message: `Game difficulty updated to ${difficulty}`,
    });
  });
  ...
  socket.on("disconnect", () => {
    ...
    for (const [roomId, room] of gameRooms.entries()) {
      if (room.players.some((player) => player.id === socket.id)) {
        console.log(
          `Player ${socket.id} was in room ${roomId}, handling departure`
        );
        handlePlayerLeaving(socket, roomId);
      }
    }
...
function sendRoomList(socket = null) {
  ...
  for (const [roomId, room] of gameRooms.entries()) {
    console.log(
      `Room ${roomId}: status=${room.gameState.status}, players=${room.players.length}/${room.maxPlayers}`
    );
...
function handlePlayerLeaving(socket, roomId) {
  ...
  if (!room) {
    console.log(
      `Room ${roomId} not found when player ${socket.id} was leaving`
    );
    return;
  }
  ...
  if (player.isHost && room.players.length > 0) {
    console.log(
      `Host left room ${roomId}, assigning new host: ${room.players[0].name}`
    );
