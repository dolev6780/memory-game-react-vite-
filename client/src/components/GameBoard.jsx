...
const GameBoard = ({
  ...
  const currentTheme = {
    ...
    turnNotificationGradient:
      gameTheme === "animals"
        ? "from-green-600 to-green-500"
        : "from-blue-600 to-blue-500",
    turnNotificationBorder:
      gameTheme === "animals" ? "border-green-300" : "border-blue-300",
  };
  ...
  useEffect(() => {
    ...
    if (isFirstRender.current && isOnline) {
      ...
      try {
        ...
        if (
          socketTheme &&
          socketTheme !== gameTheme &&
          typeof setGameTheme === "function"
        ) {
          console.log(
            `GameBoard enforcing theme: ${gameTheme} -> ${socketTheme}`
          );
  ...
  const getPlayerTurnText = () => {
    ...
    const turnText =
      getText(gameTheme, language, "playerTurn") ||
      (language === "en" ? "'s Turn" : "תורו של");
  ...
  return (
    <div
      className={`flex flex-col items-center w-full max-h-screen p-2 sm:p-3 ${
        currentTheme.container
      } rounded-lg backdrop-blur-sm shadow-lg overflow-hidden ${
        language === "he" ? "rtl" : "ltr"
      }`}
      ref={containerRef}
    ...
    >
        ...
        >
          {cards.map((card, index) => {
            ...
            const enhancedCard = {
              ...
              displayName:
                card.displayName || card.name?.he || card.name?.en || card.name,
            };
            ...
            return (
              <GameCard
                ...
                matchedByText={
                  ...
                  (language === "en" ? "Matched by" : "הותאם על ידי")
                }
              />
            );
          })}
      ...
      {gameTheme === "flags" ? (
        <a
          ...
          className="absolute bottom-0 left-0 text-gray-400"
        >
          Flags Designed by Luis_Molinero / Freepik
        </a>
      ) : null}
    </div>
  );
